...
```{r corpus summary 2, eval = FALSE}
summary(transcriptCorpus, 10)
```

We can clean up the patterns:

```{r clean up tags, eval = FALSE}
...
```{r corpus summary 3, eval = FALSE}
summary(transcriptCorpus, 10)
```

```{r table docvars, eval = FALSE}
...
```{r emotions, eval = FALSE}
canddfmRelRID[, "EMOTIONS.GLORY"]
```

```{r dotchart, eval = FALSE}
